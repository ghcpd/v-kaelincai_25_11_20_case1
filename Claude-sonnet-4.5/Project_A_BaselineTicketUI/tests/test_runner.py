"""
Test harness for Project A - Baseline Ticket UI
...
class TestHarness:
    ...
    def run_tests(self) -> Dict[str, Any]:
        ...
        print("=" * 80)
        print("BASELINE TICKET UI - TEST EXECUTION")
        print("=" * 80)
        print()
    ...
    def validate_output(
        ...
        for key in ["hierarchy_clarity_score", "scroll_length_reduction", "mis_operation_risk"]:
            ...
            # For baseline, we expect poor metrics, so high tolerance
            tolerance = 0.3
            if delta > tolerance:
                metrics_match = False
                issues.append(f"{key}: expected ~{expected_val}, got {actual_val} (delta: {delta:.2f})")
        
        # Check expected behavior (baseline will fail most of these)
        ...
        if not issues and metrics_match:
            status = "passed"
        elif actual_status == "partial_success":
            status = "partial"
        else:
    ...
    def validate_behavior(
        ...
        if expected_behavior.get("hierarchy_restructured"):
            core_at_top = [s for s in sections[:3] if s.get("type") in ["description", "priority", "status"]]
        ...
        if expected_behavior.get("collapsible_sections"):
            collapsible_found = any(s.get("collapsible") for s in sections)
            if not collapsible_found:
                unmet.append("no_collapsible_sections")
            else:
                met.append("collapsible_sections_present")
        ...
        if expected_behavior.get("default_collapsed"):
            ...
            for section_type in expected_collapsed:
                ...
                if matching and not any(s.get("collapsed") for s in matching):
                    unmet.append(f"{section_type}_not_collapsed_by_default")
        
        # Check attachment preview mode
        if expected_behavior.get("attachment_preview_mode"):
            expected_mode = expected_behavior["attachment_preview_mode"]
            attachment_sections = [s for s in sections if s.get("type") == "attachments"]
            for section in attachment_sections:
                files = section.get("files", [])
                for f in files:
                    if isinstance(f, dict):
                        actual_mode = f.get("preview_mode", "unknown")
                        if actual_mode != expected_mode and actual_mode != "side_panel":
                            unmet.append(f"attachment_preview_not_optimized")
                            break
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        print("\n" + "=" * 80)
        print("TEST SUMMARY - BASELINE TICKET UI")
        print("=" * 80)
    ...
    def save_results(self, output_dir: str):
        ...
        # Save JSON results
        results_file = os.path.join(output_dir, "results_pre.json")
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2)
        print(f"Results saved to: {results_file}")
        
        # Save detailed log
        log_file = os.path.join(output_dir, "log_pre.txt")
        with open(log_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("BASELINE TICKET UI - DETAILED TEST LOG\n")
            f.write("=" * 80 + "\n\n")
