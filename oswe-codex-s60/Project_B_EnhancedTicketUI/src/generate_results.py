...
def main():
    ...
    (results_dir / "results_post.json").write_text(json.dumps(outputs, indent=2), encoding="utf-8")
    timestamp = datetime.utcnow().isoformat()
    (logs_dir / "log_post.txt").write_text("\n".join([timestamp] + log_lines), encoding="utf-8")
    print(f"Wrote results_post.json with {len(outputs)} scenarios")
